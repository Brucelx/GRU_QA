...
trainfile = unicode('E:/git/GRU_QA/QA_system/nlpcc_train','utf-8')
...
vectorsfile_linux = unicode('/home/liuxiao/GRU/cnn/vectors_100.bin','utf-8')
outputfile_linux = unicode('/home/liuxiao/GRU/cnn/output.txt','utf-8')
paramsfile_linux = unicode('/home/liuxiao/GRU/cnn/params.txt','utf-8')

validationfile = unicode('/home/liuxiao/GRU/nlpcc_validation','utf-8')
...
def validation(validate_model, testList, test_seq, vocab, batch_size, word_embeddings, sequence_len, embedding_size):
    ...
    for i in range(questiong_number):
        ...
        if(accumulate <= n*batch_size):
            MRR = answer_rank(input_probablity, right_ans_num, answer_numbers, every_test_seq[0])
            MRR_sum += MRR
            MRR_numbers += 1
        else:break
...
#答案排序确定最好答案的位置
def answer_rank(probablity, right_ans, ans_numbers, qid):
    rank = 1
    ...
    MRR = 1.0/rank
    if(rank != 1):
        print 'qid:' + str(qid)  + ' right_answer:' + str(right_ans) + ' answer_number:' + str(ans_numbers)
        for i in range(len(probablity)):
            if(i == right_ans):
                print 'right_answer: ' + str(probablity[i])
            else:
                print str(probablity[i]) + ' '
        print '\n'
    return MRR
...
class QAMODEL(object):
    def __init__(self,word_embeddings, batch_size, sequence_len, embedding_size, filter_sizes, num_filters, learning_rate):
        ...
        #cnn单通道输入，词关联矩阵
        related_matrix = gener_matrix_leaner_transform(lstm1.dimshuffle(1, 0, 2), lstm2.dimshuffle(1, 2, 0), tparams[_p('leaner_transform', 'W')], batch_size)
        ...
        self.train_model = theano.function(
            [train_q, train_a, y, mask1, mask2],
            [self.prediction, self.cost, self.error, self.probablity],
            updates=updates
        )
...
def train():
    ...
    testList = load_test_list2(testfile)
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        prediction_ij, cost_ij, error_ij, probablity_ij = model.train_model(train_q,train_a,train_mark, mask1, mask2)
        print 'load data done ...... epoch:' + str(epoch) + ' cost:' + str(cost_ij) + ', error:' + str(error_ij)
        if epoch % validation_freq == 0:
            print 'Evaluation ......'
            MRR_result = validation(model.validate_model, testList, test_seq, vocab, batch_size, word_embeddings, sequence_len, embedding_size)
...
def predict(modelClass=QAMODEL):
    ...
    print 'start'
    testList = load_test_list2(validationfile)
    test_seq = get_test_seq(testList)
    print 'Evaluation ......'
    validation(model.validate_model, testList, test_seq, vocab, batch_size, word_embeddings, sequence_len,
               embedding_size)
    print 'end'
