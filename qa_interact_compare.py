...
version_num = '1'
trainfile = '/home/liuxiao/theano/train1_data_version_' + version_num
train0file = '/home/liuxiao/theano/train0_data_version_' + version_num
test1file = '/home/liuxiao/theano/test_data_version_' + version_num
vectorsfile = '/home/liuxiao/theano/vectors_300.bin'
paramsfile = '/home/liuxiao/theano/cnn/parameters'
validationfile = '/home/liuxiao/theano/nlpcc_validation'
#vectorsfile = '/opt/exp_data/word_vector_cn/all_dbqa.bin'

#log file
logfile_path = '/home/liuxiao/theano/cnn'
...
def validation(validate_model, testList, vocab, batch_size, words_num_dim):
    ...
    while True:
        ...
        wo_q2, wo_a2 = gener_wordoverlap(x1, x3, batch_size, words_num_dim, vocab)
        batch_scores, nouse = validate_model(x1, x2, x3, 1.0, wo_q1, wo_a1, wo_q2, wo_a2)
...
class QACnn(object):
    def __init__(self,word_embeddings, batch_size, sequence_len, embedding_size, filter_sizes, num_filters, margin_size, learning_rate):
        ...
        wo_q1, wo_a1, wo_q2, wo_a2 = T.tensor4('woq1'), T.tensor4('woa1'), T.tensor4('woq2'), T.tensor4('woa2')
        ...
        input_x3 = input_matrix1.reshape((batch_size, 1, sequence_len, embedding_size))
        input_x4 = input_matrix3.reshape((batch_size, 1, sequence_len, embedding_size))
        ...
        input_x3 = T.concatenate((input_x3, wo_q2), axis=3)
        input_x4 = T.concatenate((input_x4, wo_a2), axis=3)
        ...
        for filter_size in filter_sizes:
            ...
            image_shape = (batch_size, 1, sequence_len, embedding_size + 1)
            fan_in = np.prod(filter_shape[1:])
            fan_out = filter_shape[0] * np.prod(filter_shape[2:])
            W_bound = np.sqrt(6. / (fan_in + fan_out))
            W = theano.shared(
            ...
            conv_out = conv2d(input=input_x4, filters=W, filter_shape=filter_shape, input_shape=image_shape)
            pooled_out = pool.pool_2d(input=conv_out, ws=(sequence_len - filter_size + 1, 1), ignore_border=True,
                                      mode='max')
            pooled_active = T.tanh(pooled_out + b.dimshuffle('x', 0, 'x', 'x'))
            outputs_4.append(pooled_active)
        ...
        output_flat4 = T.reshape(T.concatenate(outputs_4, axis=1), [batch_size, num_filters_total])
        ...
        output_drop4 = self._dropout(rng, output_flat4, keep_prob)

        W = np.asarray(ortho_weight(num_filters * len(filter_sizes)))
        W_interact = theano.shared(W, borrow=True)
        self.tparams[_p('interact', 'W')] = W_interact
        self.params += [W_interact]

        qra_interact, update = theano.scan(lambda a, b, interact: T.dot(T.dot(a, interact), b),
                                     sequences=[output_drop1, output_drop2], non_sequences=W_interact)

        qwa_interact, update = theano.scan(lambda a, b, interact: T.dot(T.dot(a, interact), b),
                                           sequences=[output_drop3, output_drop4], non_sequences=W_interact)

        qra_interact = T.reshape(qra_interact, [batch_size, 1])
        qwa_interact = T.reshape(qwa_interact, [batch_size, 1])

        qra_interated = T.concatenate((output_drop1, qra_interact, output_drop2), 1)
        qwa_interated = T.concatenate((output_drop1, qwa_interact, output_drop2), 1)

        probablity_pos = T.nnet.softmax(T.dot(qra_interated, tparams['softmax_W']) + tparams['softmax_b'])
        probablity_neg = T.nnet.softmax(T.dot(qwa_interated, tparams['softmax_W']) + tparams['softmax_b'])

        # 预测正向答案和负项答案为最佳答案概率
        prediction12 = probablity_pos[:, 1]
        prediction34 = probablity_neg[:, 1]
        self.prediction12 = prediction12
        self.prediction34 = prediction34
        ...
        #Loss损失函数
        diff = T.cast(T.maximum(zero, margin - prediction12 + prediction34), dtype=theano.config.floatX)
        ...
        self.train_model = theano.function(
            [input1, input2, input3, keep_prob, wo_q1, wo_a1, wo_q2, wo_a2],
            [self.cost, self.accuracy, self.dbg_x1, self.dbg_outputs_1],
            updates=updates,

        )
        self.validate_model = theano.function(
            inputs=[input1, input2, input3, keep_prob, wo_q1, wo_a1, wo_q2, wo_a2],
            outputs=[prediction12, prediction34],
        )
...
def train():
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        wo_q1, wo_a1 = gener_wordoverlap(train_x1, train_x2, batch_size, words_num_dim, vocab)
        wo_q2, wo_a2 = gener_wordoverlap(train_x1, train_x3, batch_size, words_num_dim, vocab)
        #print train_x3.shape
        cost_ij, acc, dbg_x1, dbg_outputs_1 = model.train_model(train_x1, train_x2, train_x3, keep_prob_value, wo_q1, wo_a1, wo_q2, wo_a2)
