...
trainfile = unicode('E:/git/GRU_QA/QA_system/nlpcc_train1','utf-8')
train0file = unicode('E:/git/GRU_QA/QA_system/nlpcc_train0','utf-8')
testfile = unicode('E:/git/GRU_QA/QA_system/nlpcc_test','utf-8')
vectorsfile = unicode('E:/git/GRU_QA/QA_system/vectors_100.bin','utf-8')
outputfile = unicode('E:/git/GRU_QA/QA_system/output.txt','utf-8')
paramsfile = unicode('E:/git/GRU_QA/QA_system/params.txt','utf-8')

trainfile_linux = unicode('/home/liuxiao/GRU/DBQA/nlpcc_train1','utf-8')
train0file_linux = unicode('/home/liuxiao/GRU/DBQA/nlpcc_train0','utf-8')
testfile_linux = unicode('/home/liuxiao/GRU/DBQA/nlpcc_test','utf-8')
vectorsfile_linux = unicode('/home/liuxiao/GRU/DBQA/vectors_100.bin','utf-8')
outputfile_linux = unicode('/home/liuxiao/GRU/cnn1/output.txt','utf-8')
paramsfile_linux = unicode('/home/liuxiao/GRU/cnn/params.txt','utf-8')

validationfile = unicode('/home/liuxiao/GRU/DBQA/nlpcc_validation','utf-8')
...
def gener_matrix_leaner_transform(a, b, c, batch_size):
    result = []
    for i in range(batch_size):
        mm = T.dot(T.dot(a[i], c), b[i])
        result.append(mm)
    return result
...
class QAMODEL(object):
    def __init__(self,word_embeddings, batch_size, sequence_len, embedding_size, filter_sizes, num_filters, learning_rate, margin_value):
        ...
        #cnn单通道输入，词关联矩阵
        related_matrix_pos = gener_matrix(lstm1.dimshuffle(1, 0, 2), lstm2.dimshuffle(1, 2, 0), batch_size)
        cnn_input_pos = T.reshape(related_matrix_pos, [batch_size, 1, sequence_len, embedding_size])
        related_matrix_neg = gener_matrix(lstm1.dimshuffle(1, 0, 2), lstm3.dimshuffle(1, 2, 0), batch_size)
        cnn_input_neg = T.reshape(related_matrix_neg, [batch_size, 1, sequence_len, embedding_size])

        cnn_result_pos = self._cnn_net(tparams, cnn_input_pos, batch_size, sequence_len, num_filters, filter_sizes, embedding_size)
        cnn_result_neg = self._cnn_net(tparams, cnn_input_neg, batch_size, sequence_len, num_filters, filter_sizes,embedding_size)

        probablity_pos = T.nnet.softmax(T.dot(cnn_result_pos, tparams['softmax_W']) + tparams['softmax_b'])
        probablity_neg = T.nnet.softmax(T.dot(cnn_result_neg, tparams['softmax_W']) + tparams['softmax_b'])

        #预测正向答案和负项答案为最佳答案概率
        prediction12 = probablity_pos[:,1]
        prediction13 = probablity_neg[:,1]
        self.prediction12 = prediction12
        self.prediction13 = prediction13
        ...
        # Loss损失函数
        diff = T.cast(T.maximum(zero, margin - prediction12 + prediction13), dtype=theano.config.floatX)
        ...
        self.validate_model = theano.function(
            [train_1, train_2, train_3, mask],
            [self.prediction12, self.prediction13]
        )
